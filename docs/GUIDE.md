...
2. 进来就有一条种子消息。点底部 **Demo**（无需 API key）跑一段录好的多轮对话，所有面板会跟着动。
...
| **Demo** | 不需要 API key，按预设时间线播一段「计算 + 工具调用 + 决策」的多轮 agent loop。适合第一次看产品的人。 |
...
- **Model**：填 provider 端实际可用的模型名（与左下角"假想架构"无关）。
...
**不持久化**：steps（时间线）、KV 基线缓存、hover 状态 —— 都按"会话级"处理，刷新即清。

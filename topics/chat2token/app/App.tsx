...
const appCopy = {
  en: {
    loading: "Loading TokenTour...",
    subtitle: "From Agentic Context to KV Cache · interactive visual tutorial",
    copiedPrompt: "Copy this share link:",
    switchTitle: "Switch to Chinese",
    helpTitle: "Panel guide + suggested experiments",
    help: "Help",
    shareTitle: "Copy current messages and settings into a shareable URL",
    copied: "Copied ✓",
    share: "Share",
  ...
  zh: {
    loading: "正在加载 TokenTour...",
    subtitle: "从 Agentic Context 到 KV Cache · 交互式可视化教程",
    copiedPrompt: "复制下面的分享链接：",
    switchTitle: "Switch to English",
    helpTitle: "面板说明 + 玩法建议",
    help: "帮助",
    shareTitle: "把当前消息和设置编码到 URL 复制到剪贴板",
    copied: "已复制 ✓",
    share: "分享",
    settings: "设置 / BYOK",
    githubTitle: "在 GitHub 上查看源码",
  },
} as const;

export default function App({ initialLang = "en" }: { initialLang?: Lang }) {
  ...
  useEffect(() => {
    ...
    if (snap) {
      ...
      if (isKnownSeedState(s.systemPrompt, s.messages)) {
        s.setSystemPrompt(DEFAULT_SYSTEM_PROMPT[nextLang]);
        s.setMessages(seedMessagesFor(nextLang));
        s.clearSteps();
      }
    }
  }, []);
  ...
  const handleShare = () => {
    ...
    navigator.clipboard
      ...
      .catch(() => prompt(appCopy[lang].copiedPrompt, url));
  };

  if (!hydrated) {
    return (
      <div className="pg-root grid h-screen place-items-center text-(--color-muted)">
        {appCopy[lang].loading}
      </div>
    );
  }

  const copy = appCopy[lang];
  const langHref = lang === "zh" ? "/chat2token/playground" : "/chat2token/playground/zh";
  const langLabel = lang === "zh" ? "English" : "中文";
  const langTitle = copy.switchTitle;
    ...
    <I18nProvider lang={lang}>
      <div className="pg-root flex h-screen flex-col gap-px bg-(--pg-desk)">
        <header className="pg-chrome flex items-center justify-between px-4 py-2">
          ...
          <div className="flex items-center gap-2">
            <a className="btn" href={langHref} title={langTitle} aria-label={langTitle}>
              {langLabel}
            </a>
            <button
              className="btn"
